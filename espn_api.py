...
async def get_schedule(session: aiohttp.ClientSession) -> list[dict]:
                """Get Whitecaps schedule. Tries season=year-1 first due to ESPN quirk
                    where season=current_year returns 0 events early in the season."""
    ...
    for url in (
        ...
        log.info("Schedule (%s): events=%d", url.split("?")[-1], len(events))
        matches = [m for e in events for m in [_parse_match(e)] if m]
        if matches:
                                log.info("get_schedule: parsed %d matches", len(matches))
            return matches
    return []


async def get_standings(session: aiohttp.ClientSession) -> dict | None:
                """Get MLS standings via /apis/v2/ endpoint (site/v2 returns empty {})."""
