...
# Normalize stat key names from the v2 standings API to the names
# expected by embeds.py (build_standings_embed).
_STAT_KEY_MAP = {
            "ppg": "pointsPerGame",
}
...
async def get_schedule(session: aiohttp.ClientSession) -> list[dict]:
            """Get the Whitecaps schedule.

                ESPN's team schedule endpoint returns 0 events when queried with the
                    current calendar year early in the season (before data is populated).
                        We try season=current_year-1 first (which ESPN re-maps to the active
                            season), then season=current_year, then no season param as a fallback.
                                """
    year = datetime.now(timezone.utc).year

    # ESPN quirk: querying ?season=<prev_year> returns the CURRENT active
    # season's full schedule early in the year (e.g. in Feb 2026, season=2025
    # returns all 2026 MLS games). We try previous year first.
    for url in (
...
async def get_standings(session: aiohttp.ClientSession) -> dict | None:
            """Get MLS standings.
                ...
                Uses the /apis/v2/ endpoint which reliably returns current standings,
                    unlike /apis/site/v2/ which returns empty {} for the standings resource.
                        Stat keys from the v2 API are normalized to the names expected by embeds.py
                            (e.g. 'ppg' -> 'pointsPerGame').
                                """
    data = await _fetch(session, STANDINGS_URL)
    if not data:
                    log.warning("get_standings: no data from %s", STANDINGS_URL)
        return None

    log.info("Standings: top keys=%s", list(data.keys()))

    children = data.get("children") or data.get("groups") or []
    if not children:
                    log.info("Standings: no 'children' or 'groups'. Keys: %s", list(data.keys()))
        return None

    standings = {}
    for group in children:
                    conf_name = group.get("name", "Conference")
        # v2 endpoint nests entries under group.standings.entries
        raw_entries = (
        ...
        for entry in raw_entries:
            ...
            for s in entry.get("stats", []):
                                    raw_key = s.get("name", "")
                                    # Normalize key names to match what embeds.py expects
    ...
    if standings:
                    log.info("get_standings: parsed %d conference(s)", len(standings))
        return standings
    return None
                    ...
                    try:
                                        async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                                                                ...
                                                                if status == 200:
                                                                                            ...
                                                                                            counts = {}
                                                                                            for k in ("events", "children", "groups", "entries"):
